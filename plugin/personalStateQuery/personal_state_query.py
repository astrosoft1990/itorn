...
def handle_msg(msg: QQMessage):
    try:
        torn_number = msg.message.split("#")[1]
        if len(torn_number) == 0:
            torn_number = utils.split_torn_numner(msg.sender_group_card)
        if torn_number == '0':
            raise Exception
        test_url(torn_number, msg.group_number, msg.sender_number)
